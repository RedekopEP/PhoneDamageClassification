...
class MVCNN(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        # )
        self.classifier_group = nn.Sequential(
                nn.Dropout(0.4),
                nn.Linear(fc_in_features, 2048),
                nn.ReLU(inplace=True),
                nn.Dropout(0.4),
    ...
    def forward(self, inputs):  # inputs.shape = samples x views x channels x height x width
        ...
        outputs_group = self.classifier_group(pooled_views)
        outputs_class = self.classifier_class(outputs_group)
        return outputs_group, outputs_class
...
def main():
    ...
    X_train = []
    with open('/home/sysadmin/autogradingML/phone_sem_seg/stage1_experiment/x_train_cut.csv', 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            if 'image_name' not in row[0]:
                X_train.append(row)
    y_train = []
    with open('/home/sysadmin/autogradingML/phone_sem_seg/stage1_experiment/y_train_cut.csv', 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            if 'class' not in row[0]:
                y_train.append(row)
    X_test = []
    with open('/home/sysadmin/autogradingML/phone_sem_seg/stage1_experiment/x_test_cut.csv', 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            if 'image_name' not in row[0]:
                X_test.append(row)
    y_test = []
    with open('/home/sysadmin/autogradingML/phone_sem_seg/stage1_experiment/y_test_cut.csv', 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            if 'class' not in row[0]:
                y_test.append(row)
    ...
    for epoch in range(epoch_start, config["epochs"]):
        ...
        try:
            ...
            for (imgs,  y_group, y_class) in train_loader:
                ...
                logits_group, logits_class = model(imgs)

                _loss = loss(logits_group, y_group) #+ loss(logits_class, y_class)
