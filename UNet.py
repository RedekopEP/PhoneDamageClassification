...
class double_conv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.conv = nn.Sequential(
            ...
            nn.BatchNorm2d(out_ch), #nn.InstanceNorm2d(out_ch),# nn.BatchNorm2d(out_ch),
            nn.ReLU(inplace=True)
        )
...
class inconv_drop(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(inconv_drop, self).__init__()
        self.conv = double_conv(in_ch, out_ch)
...
class down_drop(nn.Module):
    ...
    def forward(self, x):
        x = F.dropout2d(self.mpconv(x), p=0.25, training=True)
        return x
...
class UNet2D(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, activation=False):
